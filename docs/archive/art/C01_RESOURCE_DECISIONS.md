...
> **【已归档 · 2026-09-08】** 本文的 Foozle CC0 栅格决议先被 v2 自绘取代，再由 sourced-cc0-ccby-v1（外部 CC0/CC-BY 素材 + `tools/build_sourced_assets.py` 派生管线）取代。现行资产记录以 `docs/current/art/ASSET_CATALOG.md` §4.6/§9、`ART_ASSET_REGISTRY.csv`（`C01/C02_SOURCED_PRESENTATION` 行）与 `ASSET_LICENSE_LEDGER.csv` 为准。本文仅作来源沿革存档。
>
