...
`docs/current/engineering/M4_ASSET_SPEC.md`：32×32 像素基线 / 640×360 整数倍缩放 / 调色板与轮廓规则 /
...
- `docs/current/art/CREDITS.md` 建立：Godot MIT + Ark Pixel OFL（真实接入项）+ Buch 试验性说明。
...
- 机制层（隐匿/治疗光环/双相抗性/掩体阻挡/侦测与孢子装置）详见 docs/archive/milestones/M4_CONTENT_NOTES.md。
...
- 机制层（召唤敌/精英再生/Boss 2 暴露核心窗口）详见 docs/archive/milestones/M4_CONTENT_NOTES.md M4-D 段。
