...
> 当前视觉权威：[ART_STYLE_BASELINE.md](../art/ART_STYLE_BASELINE.md)；当前美术生产权威：[ART_PRODUCTION_PLAN.md](../art/ART_PRODUCTION_PLAN.md)
> 冲突规则：本文仅保留技术接入参考；像素比例、色板、构图与后续关卡风格冲突时，以 `ART_STYLE_BASELINE.md` 为准。
...
- 发布构建的 credits 必须包含 Godot（MIT）与全部第三方资产归因（PRD §18.6），见 `docs/current/art/CREDITS.md`。
