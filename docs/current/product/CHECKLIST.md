...
>
> 当前执行权威：[PROJECT_EXECUTION_BASELINE.md](PROJECT_EXECUTION_BASELINE.md)；美术生产权威：[ART_PRODUCTION_PLAN.md](../art/ART_PRODUCTION_PLAN.md)。
>
...
- [x] 建立资产下载、原始许可证、hash、credits 归档流程（NEXT_PHASE 管线验证 + M4-A docs/current/art/CREDITS.md）
- [x] 完成 C01 灰盒地图
...
> 2026-09-05：M3 主要代码与内容基线已落地，状态 PARTIAL。详见 [M3_CHAPTER_NOTES.md](../../archive/milestones/M3_CHAPTER_NOTES.md)。最终性能聚合、确定性回归、suspend 一致性、编辑器导入和人工退出评审未确认，因此以下 M3 门禁保持未勾选。
...
> 详见 [M3_POLISH_NOTES.md](../../archive/milestones/M3_POLISH_NOTES.md) 与 [M3_POLISH_TODO.md](../../archive/plans/M3_POLISH_TODO.md)。
...
- [x] Foozle 原包、CC0、SHA-256、派生清单、Ledger、Registry、Credits 收口
- [x] 主理人确认视觉方向并锁定为后续关卡基准
- [ ] 外部玩家识别测试与完整 Visual-QA/Shipping 门禁（仍按 ART_PRODUCTION_PLAN 执行）
...
- **阶段说明**：[NEXT_PHASE.md](../../archive/milestones/NEXT_PHASE.md)
...
- [x] 美术技术规范锁定（[M4_ASSET_SPEC.md](../engineering/M4_ASSET_SPEC.md)：32×32/640×360/调色板/轮廓/帧/命名/导入/许可证）
- [x] C01 切片资产 16 张（项目自有原创，`tools/gen_c01_sprites.py` 确定性生成）
- [x] 运行时接入（`ArtLibrary` + 强制程序化回退：C01 地形、3 塔、2 敌、岚舟英雄、2 FX 帧条、4 HUD 图标）
- [x] 台账 +16 行（Project-owned，含 sha256）与 [CREDITS.md](../art/CREDITS.md)（仅真实接入项）
- [x] 验证：导入 0 error / validate PASS / tests 117/117 / i18n missing=0 / C01 autoplay ticks=7017 与 steady 7080 均与基线一致 / perf 144fps avg
- **记录**：[M4_ASSET_PIPELINE.md](../engineering/M4_ASSET_PIPELINE.md)；未做项（其余塔/敌/Boss 精灵、正式音频、色弱贴图重映射、terrain_land_b 未接入）见该文 §6
...
- [x] 台账 +41 行并修复 df6b434 遗留列数不一致；[CREDITS.md](../art/CREDITS.md) 与 [M4_ASSET_PIPELINE.md](../engineering/M4_ASSET_PIPELINE.md) 同步
...
- **记录**：[M4_ASSET_PIPELINE.md](../engineering/M4_ASSET_PIPELINE.md) M4-B 段；未做项（guardian 精灵、精英/Boss 阶段变体、FX/UI 色弱变体、正式音频、land_b 未接入）见该文 §6
...
- [x] C09–C12 balance steady 1×/3× 确定性对照与 perf 报告生成（C04–C12 regression/perf 聚合均 failures=0 PASS；见 [M4_CONTENT_NOTES.md](../../archive/milestones/M4_CONTENT_NOTES.md) §5）
- **记录**：[M4_CONTENT_NOTES.md](../../archive/milestones/M4_CONTENT_NOTES.md)；本节原未做项 C13–C14/Boss 2/精英敌已在 M4-D 完成。
...
- **历史记录**：[M4_CONTENT_NOTES.md](../../archive/milestones/M4_CONTENT_NOTES.md) M4-D 段。旧“后续从 C15 开始”方向已废止；只有 M2-GATE 和后续 M3-GATE 均由人类主理人正式批准后，才允许恢复 M4 批量内容。
...
- [x] 明确资产成熟度：C01 最终风格表现层已获主理人采用并登记为非占位 `Integrated`；C02–C14 仍是可替换 provisional/placeholder；全项目尚非 Shipping
...
- **记录**：[PLAYER_EXPERIENCE_AUDIT.md](../quality/PLAYER_EXPERIENCE_AUDIT.md)。此前以自动化/数据覆盖替代玩家流程验收的口径已废弃；恢复 C15+ 前先补玩家外壳。
...
| 2026-09-06 | C01 Foozle CC0 栅格表现层完成并获主理人视觉批准；建立 ART_STYLE_BASELINE.md 为唯一最终风格权威，C01 作为黄金样板，C02 及后续关卡必须沿用；旧 C01 文档收敛为实现与资源索引 | 用户确认“美术风格不错我采用了，最后定版这个风格，以后的关卡都用这个风格” | 项目主理人 |
