...
> **Supersedes**：`ASSET_AUDIO_RESEARCH_DRAFT.md`(v0.1)
...
1. 引用 `ART_STYLE_BASELINE.md` 中已批准的正式风格，并提供候选池供后续关卡生产选用。
...
**【Policy】** 2026-09-06，项目主理人采用 C01 当前 Foozle CC0 派生栅格像素风格，并指定 C02 及后续全部关卡沿用。完整规则只维护在 `ART_STYLE_BASELINE.md`，C01 实现细节见 `C01_STYLE_BIBLE.md`。

| 维度 | 锁定 |
|---|---|
| 基准样板 | C01「离港火线」完整 Title → Campaign → Briefing → Battle → Result 流程 |
| 视觉主体 | 真实可辨认的栅格像素精灵；禁止用圆/线/多边形拼最终塔、敌人、舰船或建筑 |
| 像素密度 | 32×32 逻辑网格；普通单位显示约 32–64 px；塔可用 64–96 px 源帧；Boss 可更大 |
| 投影视角 | 战斗正交俯视；流程背景可使用同语言的海报式远景 |
| 色板关系 | 冷青灰暮潮世界 + 暖珊瑚/余烬己方焦点 + 低饱和铜木材质 |
| 描边 | 1–2 px 深色外轮廓；忙碌战斗仍能区分敌、塔、道路和舰队 |
| 渲染 | 640×360；Nearest；像素逐帧；禁止线性模糊和跨风格素材混入 |
| 程序绘制边界 | 仅用于雾、暗角、状态、范围、辉光和路径激活微光；不得主导世界主体 |

### 1.2 UI 视觉语言

- Ark Pixel CJK 字体与硬边像素布局；
- 炭黑半透明面板、羊皮文字、暖珊瑚主 CTA；
- 同屏只保留一个最强主操作；
- Campaign 按钮附着于真实关卡地标，不使用抽象流程图主导画面；
- Result 通过灯塔亮/灭、舰队通过/受阻表达胜负；
- 禁止默认 Godot 主题、玻璃拟态、柔和渐变卡片和木卷轴中世纪 UI。

### 1.3 塔视觉识别

| 塔 | 视觉锚点 |
|---|---|
| 针轨弩台 | C01 黄金样板：机械弩 + 稳定塔基 + 暖珊瑚己方高光 |
| 余烬喷井 | 喷口边缘结炭，内部琥珀光，喷发轮廓明确 |
| 回声桩阵 | 两根低矮桩柱，中间弦线/潮汐脉冲 |
| 风帆机巢 | 编织帆布 + 木骨架，可展开翼 |
| 铸潮砧塔 | 重型砧座，周围散落碎片 |
| 棱镜苗圃 | 像素化多面体，以硬边高光表现折射，禁止平滑矢量感 |

### 1.4 章节视觉递进

章节可以增加辅色与地标，但不得改变 C01 锁定的像素密度、视角、轮廓、暖己方/冷威胁关系和栅格主体原则。

| 章节 | 主色调 | 视觉变化 |
|---|---|---|
| 序章 + 第一章「盐风浅滩」 | 冷青灰 + 珊瑚暖光 | 海雾、礁石、旧港木构、迁徙舰队 |
| 第二章「玻璃沼泽」 | 冷青灰 + 蓝紫/孢绿辅色 | 玻璃质、孢光、扭曲倒影 |
| 第三章「沉钟群岛」 | 深青 + 锈铜辅色 | 钟声雕刻、藤壶、青苔 |
| 第四章「无昼海眼」 | 黑紫 + 余烬红辅色 | 永恒暮色、余烬残光 |
