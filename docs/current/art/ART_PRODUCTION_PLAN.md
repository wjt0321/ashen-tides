...
> 建立日期：2026-09-06
> 状态：Active
> 最终视觉基线：[ART_STYLE_BASELINE.md](ART_STYLE_BASELINE.md)
> 候选与许可证上位依据：[ASSET_CATALOG.md](ASSET_CATALOG.md)
> 项目顺序依据：[PROJECT_EXECUTION_BASELINE.md](../product/PROJECT_EXECUTION_BASELINE.md)
...
- `ART_STYLE_BASELINE.md`：2026-09-06 项目主理人批准的最终视觉方向；C01 为黄金样板，后续所有关卡必须沿用。
- `ASSET_CATALOG.md`：候选来源、许可与生命周期政策；风格描述冲突时服从 `ART_STYLE_BASELINE.md`。
...
只有 `docs/current/quality/gates/M2-GATE.yaml` 在当前有效 revision 上正式 `PASSED` 后，才能扩 C04–C08；C01–C03 Player-verified 只是必要条件之一。按“关卡资产包”交付，不按“先画全敌人后再接入”交付。
...
- `M4_ASSET_PIPELINE.md` 中旧 C01–C14 程序生成资产记录属于历史接入证据。C01 已由 `C01_FOOZLE_RASTER_PRESENTATION` 取代并登记为 `Integrated`、`placeholder=false`；C02–C14 仍统一按 `Integrated placeholder` 管理。
